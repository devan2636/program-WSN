...
int thisAddress = 202; // alamat yang digunakan oleh node
int headerFrom = 203; // alamat yang digunakan oleh node
int headerTo = 201; //alamat yang akan dituju
...
//[9] Program ketika gateway mode receive
  ...
  if (rf95.recv(buf, &len)){
    dataIn = (char*) buf;
    if (dataIn == "Q"){
  // Mengirim command ke node C
      rf95.setHeaderTo(headerForm);
      String message = "W";
      ...
      String message = kirim + " B " + (String)suhu + " " + (String)kelembapan + " "; // Data yang akan dikirimkan
      message.toCharArray(buff, len);
